...
function initMap(): void {
  ...
  stationData.forEach(({
    ...
    const infoString = `<h3>${code}</h3>${name}<br>${cityName}, ${stateName}<br>Boading : ${boardingFreq}<br>Deboarding : ${departureFreq}<br>Halting : ${asHalt || 0} trains<br>As Non Halting : ${asNonHalt || 0} trains`;
    addMarker(
      location,
      {
        url:
